...
def assign_weapon_point_value(weapon):
    ...
    if (weapon.get_on_hit_effect() is not None and weapon.get_on_hit_effect().get_damage_per_turn() is not None):
        # total_pv += (1 + weapon.get_on_hit_effect().get_turns_left()/10)
        total_pv += 1000 * weapon.get_on_hit_effect().get_damage_per_turn()
    if (weapon.get_splash_radius() > 1):
        total_pv += 2 ** (weapon.get_splash_radius())
    if (weapon.get_range() > 1):
        total_pv += 5 * weapon.get_range()
    total_pv += 0 * weapon.stats.get_percent_experience_change() * (1 + weapon.stats.get_flat_experience_change()) 
    return total_pv
...
def assign_shoes_point_value(shoes):
	total_pv = 0
	total_pv += max(1, shoes.stats.get_flat_speed_change()*50) * (1 + shoes.stats.get_percent_speed_change())
	total_pv += max(1, shoes.stats.get_flat_health_change()) * (1 + shoes.stats.get_percent_health_change()) / 2
	total_pv += max(1, shoes.stats.get_flat_defense_change()) * (1 + shoes.stats.get_percent_defense_change())
	total_pv += shoes.stats.get_flat_regen_per_turn() * 4
	return total_pv

def assign_hat_and_accessory_point_value(accessory):
    total_pv = 0
    total_pv += 0 * (1 + accessory.stats.get_flat_experience_change()) * (1 + accessory.stats.get_percent_experience_change())
